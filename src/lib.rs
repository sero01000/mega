...
impl Mega {
    ...
    fn prepare_key(pwd_str: &str) -> [u8;16] {
        ...
        let mut pkey:[u8; 16] = [0x93, 0xC4, 0x67, 0xE3, 0x7D, 0xB0, 0xC7, 0xA4, 0xD1, 0xBE, 0x3F, 0x81, 0x01, 0x52, 0xCB, 0x56];
        for _it in 0..65536{
    ...
    fn hmac_hash(password:&str,salt:&str)->Result<[u8;32], std::io::Error>{
        if let Ok(salt_work) = base64::decode(salt){
            ...
            pbkdf2::derive(PBKDF2_ALG, NONZERO100000.unwrap(), &salt_work,password.as_bytes(), &mut result);
            Ok(result)
        }
        else{
            Err(std::io::Error::new(std::io::ErrorKind::Other, "Error in salt"))
        }
    }
    ...
    pub fn login_email_hash(email:&str,hash:&str,url:&str)->Result<String,std::io::Error>{
        let post_data = format!("[{{\"a\": \"us\", \"user\": \"{}\", \"uh\": \"{}\"}}]",email,hash);
        if let Ok(req2) = Self::post_mega(&url,post_data){
            if req2.contains("[-13]"){
                Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, "Wrong email or password"))
            }
            else if req2.contains("\"csid\":\""){
                Ok(req2)
            }
            else{
                Err(std::io::Error::new(std::io::ErrorKind::Other, format!("Else: {:?}",req2)))
            }
        }
        else{
            Err(std::io::Error::new(std::io::ErrorKind::NotConnected, "Connection error"))
        }
    }

    pub fn login(email:&str,password:&str)->Result<String,std::io::Error>{
        let post_data = format!("[{{\"a\":\"us0\",\"user\":\"{}\"}}]",email);
        let url = format!("https://g.api.mega.co.nz/cs?id={}",rand::thread_rng().gen_range(0..10000000));
            ...
            if req1.contains("\"v\":1"){
                ...
                if let Some(salt) = Self::split_salt("\"s\":\"","\"",&req1){
                    if let Ok(hash_parsed) = Self::hmac_hash(password,&salt){
                        let hash = base64::encode_config(&hash_parsed[16..32], base64::URL_SAFE_NO_PAD);
                        Self::login_email_hash(email,&hash,&url)
                    }
                    else{
                    ...
                    Err(std::io::Error::new(std::io::ErrorKind::InvalidData, "Parsing salt"))
                }
                
            }
            else{
                Err(std::io::Error::new(std::io::ErrorKind::NotFound, "No version in post data"))
            }
        }
        else{
            Err(std::io::Error::new(std::io::ErrorKind::NotConnected, "Connection error"))
